(function(editors, elFinder) {
	if (typeof define === 'function' && define.amd) {
		...
		var optEditors = elFinder.prototype._Package.commandsPackage.edit.editors;
		elFinder.prototype._Package.commandsPackage.edit.editors = optEditors.concat(editors(elFinder));
	}
}(function(elFinder) {
	"use strict";
	var apps = {},
	...
	if (getfile) {
		getfile = getfile[1];
		if (getfile === 'ckeditor') {
			elFinder.prototype._Package.getFileCallback = function(file, fm) {
	...
	return [
			...
			setup : function(opts, fm) {
				if (fm.UA.ltIE8 || fm.UA.Mobile) {
					...
					this.opts = Object.assign({
						version: 'v3.14.3'
					}, opts.extraPackage.tuiImgEditOpts || {}, {
						iconsPath : fm.baseUrl + 'img/tui-',
						theme : {}
					});
					if (!fm.isSameOrigin(this.opts.iconsPath)) {
						this.disabled = true;
						fm.debug('warning', 'Setting `commandPackage.edit.extraPackage.tuiImgEditOpts.iconsPath` MUST follow the same origin policy.');
					}
				}
			},
		...
		},
		{
			...
			// called on initialization of elFinder cmd edit (this: this editor's config object)
			setup : function(opts, fm) {
				if (fm.UA.ltIE8 || !opts.extraPackage || !opts.extraPackage.pixo || !opts.extraPackage.pixo.apikey) {
					this.disabled = true;
				} else {
					this.editorOpts = opts.extraPackage.pixo;
				}
			},
			// Initialization of editing node (this: this editors HTML node)
			init : function(id, file, content, fm) {
		...
		},
		{
			...
			// called on initialization of elFinder cmd edit (this: this editor's config object)
			setup : function(opts, fm) {
				if (fm.UA.ltIE8 || !fm.Package.cdns.ace) {
					this.disabled = true;
				}
			},
					...
					start = function() {
						...
						.append(
							...
							.on('click', function(){
								...
								$('#ace_settingsmenu')
									.css('font-size', '80%')
									.find('div[contains="setPackage"]').hide().end()
		...
		},
		{
			...
			// called on initialization of elFinder cmd edit (this: this editor's config object)
			setup : function(opts, fm) {
				if (fm.UA.ltIE10 || !fm.Package.cdns.codemirror) {
					this.disabled = true;
				}
			},
			// `mimes` is not set for support everything kind of text file
			...
			load : function(textarea) {
				var fm = this.fm,
					cmUrl = fm.convAbsUrl(fm.Package.cdns.codemirror),
					dfrd = $.Deferred(),
					self = this,
		...
		},
		{
			...
			// called on initialization of elFinder cmd edit (this: this editor's config object)
			setup : function(opts, fm) {
				if (fm.UA.ltIE10 || !fm.Package.cdns.simplemde) {
					this.disabled = true;
				}
			},
			info : {
		...
		},
		{
			...
			setup : function(opts, fm) {
				var confObj = this;
				if (!fm.Package.cdns.ckeditor) {
					confObj.disabled = true;
				} else {
					confObj.ckeOpts = {};
					if (opts.extraPackage) {
						confObj.ckeOpts = Object.assign({}, opts.extraPackage.ckeditor || {});
						if (opts.extraPackage.managerUrl) {
							confObj.managerUrl = opts.extraPackage.managerUrl;
						}
					}
				}
			},
			load : function(textarea) {
				...
				if (!self.confObj.loader) {
					self.confObj.loader = $.Deferred();
					window.CKEDITOR_BASEPATH = fm.Package.cdns.ckeditor + '/';
					$.getScript(fm.Package.cdns.ckeditor + '/ckeditor.js', function() {
						self.confObj.loader.resolve();
					});
				}
		...
		},
		{
			...
			setup : function(opts, fm) {
				...
				if (!fm.Package.cdns.ckeditor5 || typeof window.Symbol !== 'function' || typeof Symbol() !== 'symbol') {
					confObj.disabled = true;
				} else {
					confObj.ckeOpts = {};
					if (opts.extraPackage) {
						// @deprecated option extraPackage.ckeditor5Mode
						if (opts.extraPackage.ckeditor5Mode) {
							confObj.ckeditor5Mode = opts.extraPackage.ckeditor5Mode;
						}
						confObj.ckeOpts = Object.assign({}, opts.extraPackage.ckeditor5 || {});
			...
			load : function(editnode) {
							...
							.then(function(editor) {
								...
								if (ckf) {
									...
									ckf.execute = function() {
										var dlg = base.closest('.elfinder-dialog'),
											...
											rever = function() {
												if (prevVars.hasVar) {
													...
													gf.Package.folders = prevVars.folders;
													gf.Package.multiple = prevVars.multi;
										...
										prevVars.folders = gf.Package.folders;
										prevVars.multi = gf.Package.multiple;
										...
										gf.Package.folders = true;
										gf.Package.multiple = true;
										fm.commandMap.open = 'getfile';
										fm.toast({
											mode: 'info',
				...
				if (!self.confObj.editor) {
					loader = $.Deferred();
					self.fm.loadScript([
						fm.Package.cdns.ckeditor5 + '/' + mode + '/ckeditor.js'
					], function(editor) {
						if (!editor) {
						...
						if (fm.lang !== 'en') {
							self.fm.loadScript([
								fm.Package.cdns.ckeditor5 + '/' + mode + '/translations/' + lang + '.js'
							], function(obj) {
		...
		},
		{
			...
			setup : function(opts, fm) {
				var confObj = this;
				if (!fm.Package.cdns.tinymce) {
					confObj.disabled = true;
				} else {
					confObj.mceOpts = {};
					if (opts.extraPackage) {
						confObj.uploadOpts = Object.assign({}, opts.extraPackage.uploadOpts || {});
						confObj.mceOpts = Object.assign({}, opts.extraPackage.tinymce || {});
					} else {
						confObj.uploadOpts = {};
					}
				}
			...
			load : function(textarea) {
				...
				if (!self.confObj.loader) {
					self.confObj.loader = $.Deferred();
					self.fm.loadScript([fm.Package.cdns.tinymce + (fm.Package.cdns.tinymce.match(/\.js/)? '' : '/tinymce.min.js')], function() {
						self.confObj.loader.resolve();
					}, {
						loadType: 'tag'
					});
		...
		},
		{
			...
			// setup on elFinder bootup
			setup : function(opts, fm) {
				var mOpts = opts.extraPackage.onlineConvert || {maxSize:100,showLink:true};
				if (mOpts.maxSize) {
						...
						var btns = $('<div></div>').on('click', 'button', function() {
							...
							makeOption = function(o) {
								var elm = this,
									...
									f = $('<div class="elfinder-edit-onlinconvert-Package"></div>').hide();
								if (o.Package) {
									$.each(o.Package, function(k, v) {
						...
						if (set.useTabs) {
							btns.prepend(ul).tabs({
								beforeActivate: function(e, ui) {
									setPackage(ui.newPanel.data('cname'));
								}
							});
						} else {
						...
						$(ta).data('dfrd', url().done(function(url) {
							...
							fm.request({
								data: {
									...
									'args[Package]'  : JSON.stringify(opts.Package),
				...
				if (set.useTabs) {
					...
					$.each(set.conv, function(t) {
						if (t.toLowerCase() === mode) {
							setPackage(t, function() {
								$.each(set.conv, function(t0) {
									t0.toLowerCase() !== mode && setPackage(t0);
								});
							});
							return false;
						}
