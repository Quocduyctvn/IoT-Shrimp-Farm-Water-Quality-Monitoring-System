...
$(".js-print-health-cert").click(function (ev) {
	...
	var printAndUpdate = function () {
		window.print();
		$.post(`/CompanyPatient/PrintAndCount?patientId=${id}`);
	}
	showLoading();
...
$(".js-view-result").click(function (ev) {
	...
	$.getJSON(url, function (res) {
		...
		var pHistories = res.patientHistories.filter(x => x.examDate);
		var listRow = ['<tr><td colspan="13" class="text-center">Không có kết quả khám</td></tr>']
		if (pHistories.length) {
			var listRow = pHistories.map((item, i) => {
...
//
// Cụm chức năng liên quan đến update ngày khám hàng loạt
//
$(".js-check-all").click(function (ev) {
	$(".js-check-one").prop("checked", this.checked);
	$(this.dataset.toggle).prop("disabled", !this.checked);
});

const CHECK_ONE_COUNT = $(".js-check-one").length;
$(".js-check-one").click(function (ev) {
	let chkCnt = $(".js-check-one:checked").length;

	if (chkCnt > 0 && $(this.dataset.toggle).prop("disabled")) {
		$(this.dataset.toggle).prop("disabled", false);
	}
	$(".js-check-all").prop("checked", chkCnt == CHECK_ONE_COUNT);
});

$(".js-open-model-update-exam-date").click(function (ev) {
	let chkCnt = $(".js-check-one:checked").length;

	var checkedIds = Array.from($(".js-check-one:checked"))
		.map(i => $(i).closest("tr").attr("data-id"));
	$("#PatientIds").val(checkedIds.join(","));

	if (checkedIds.length == 0) {
		Toast.danger("Dữ liệu đã chọn không hợp lệ!");
		return;
	}

	var modal = new Modal("UpdateExamDate", `Cập nhật ngày khám của ${chkCnt} người`);
	modal.show();
});

$(".js-btn-update-exam-date").click(function (ev) {
	if (!$("#ExamDate").val()) {
		Toast.danger("Chưa chọn ngày khám để cập nhật!");
		return;
	}
	if (!$("#PatientIds").val()) {
		Toast.danger("Dữ liệu đã chọn không hợp lệ!");
